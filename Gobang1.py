...
def manPlay(connfd):
    p = win.getMouse()
    simsendmes(p.getX(), connfd)
    simsendmes(p.getY(), connfd)
...
def main(connfd):
    ...
    while (not is_end):
        ...
        if (start == ai):
            ...
            AI_locationx = simrecvmes(connfd)
            AI_locationy = simrecvmes(connfd)
            go(AI_locationx, AI_locationy)
        else:
            notice.setText("请你下棋...")
            manPlay(connfd)
        start = 3 - start
