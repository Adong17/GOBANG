...
def manPlay(connfd):
    ...
    if (Restart(x, y) or Quit(x, y)): return
    x = round(x / 30)
    y = round(y / 30)
    if (downOk(x, y)):
        go(x, y)
...
# 落下一子并且判断游戏是否结束
def go(x, y):
    global is_end, AI_locationx, AI_locationy
...
def main(tcp_socket):
    ...
    # p = win.getMouse()
    px = simrecvmes(tcp_socket)
    py = simrecvmes(tcp_socket)
    while (not whoStart(px,py) and not Quit(px,py)):
        # p = win.getMouse()
        px = simrecvmes(tcp_socket)
        py = simrecvmes(tcp_socket)
    while (not is_end):
        ...
        if (start == ai):
            ...
            AI1()
            simsendmes(AI_locationx, tcp_socket)
            time.sleep(0.5)
            simsendmes(AI_locationy, tcp_socket)
        else:
            notice.setText("请你下棋...")
            manPlay(tcp_socket)
        start = 3 - start
        if (RESTART_FLAG):
            ...
            px = simrecvmes(tcp_socket)
            py = simrecvmes(tcp_socket)
            while (not Restart(px,py) and not Quit(px,py)):
                # p = win.getMouse()
                px = simrecvmes(tcp_socket)
                py = simrecvmes(tcp_socket)
            while (not whoStart(px, py)and not Quit(px,py)):
